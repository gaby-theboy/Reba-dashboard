...
import { PLAN_COLORS, getPlanColor } from '../../constants/colors';
...
const PlanChart = React.memo(({
    ...
    const gradients = useMemo(() => (
        activePlans?.map((plan, index) => {
            ...
            return (
                <linearGradient key={`gradient-plan-${plan}`} id={`gradient-plan-${plan}`} x1="0" y1="0" x2="0" y2="1">
    ...
    // Create a stable plan-to-color mapping
    const planColorMap = useMemo(() => {
        const map = {};
        activePlans?.forEach((plan, index) => {
            map[plan] = getPlanColor(index);
        });
        return map;
    }, [activePlans]);
    ...
    if (!data || data.length === 0) {
        return (
            <ChartWrapper
                title="Plan Distribution Trends"
                subtitle="Count breakdown by plan"
                icon={Users}
                        ...
                        {activePlans?.map((plan) => (
                            visiblePlans[plan] && (
                                <Area
                                    ...
                                    dataKey={plan}
                                    type="monotone"
                                    fill={`url(#gradient-plan-${plan})`}
                                    stroke={planColorMap[plan]}
                                    name={plan}
                                    strokeWidth={2}
                                />
                            )
